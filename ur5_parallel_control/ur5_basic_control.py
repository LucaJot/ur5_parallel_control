...
class Ur5JointController(Node):
    def __init__(self, v_cm=5, f_update=1):  # Max v = 25 cm/s
    ...
    def send_joint_command(
        ...
        jt_msg.header.stamp = self.get_clock().now()
        jt_point = JointTrajectoryPoint()
        jt_point.positions = new_target
        jt_point.time_from_start = Duration(nanosec=int(duration * 1e9) + 10000)
